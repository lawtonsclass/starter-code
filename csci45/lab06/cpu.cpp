...
unsigned int CPU::fetch(unsigned char address) const {
  // FIXME: go into the instruction memory and fetch the binary form
  // of the instruction at the given address
}

Opcode CPU::decodeOpcode(unsigned char encoded_opcode) const {
  // FIXME: decode the binary opcode and return which Opcode it was:
  //    0 -> LOAD
  //    1 -> STORE
  //    2 -> MOVREG
  //    3 -> MOVIMM
...
Register CPU::decodeRegister(unsigned char encoded_register) const {
  // FIXME: decode the binary register and return which Register it was:
  //   0 -> Reg0
  //   1 -> Reg1
  //   2 -> Reg2
  //   3 -> Reg3
...
Instruction CPU::decode(unsigned int encoded_instruction) const {
  ...
  // FIXME: extract the opcode bits from encoded_instruction, decode
  // them, and put the Opcode in i

  // FIXME: extract the dest bits from encoded_instruction, decode
  // them, and put the Register in i

  // FIXME: extract the operand1 bits from encoded_instruction, decode
  // them, and put the Register in i

  // FIXME: extract the operand2 bits from encoded_instruction, decode
  // them, and put the Register in i

  // FIXME: extract the immediate bits from encoded_instruction, and
...
void CPU::runProgram() {
  // FIXME: set PC to 0--that's where the program starts
  ...
  while (true) {
    // Step 1: Fetch
    // FIXME: Get the current encoded instruction out of PC

    // FIXME: Increment PC to point to the next instruction
    ...
    // Step 2: Decode
    // FIXME: Figure out what kind of instruction it is
    ...
    if (instruction.opcode == LOAD) {
      // FIXME: execute this instruction to completion
