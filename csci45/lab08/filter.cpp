...
extern "C" void grayVFP(char* pixelsStart, int numPixels);
extern "C" void darkenNEON(char* pixelsStart, int numPixels);
...
int main(int argc, char* argv[]) {
  ...
  for (int row = 1; row <= height; row++) {
    ...
    if (string(argv[1]) == "gray")
      grayVFP(rowStart, numberOfPixelsToProcess);
    else
      darkenNEON(rowStart, numberOfPixelsToProcess);
