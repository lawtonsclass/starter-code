...
int main() {
  ...
  for (int y = 12; y >= -11; y--) {
    for (int x = -38; x <= 39; x++) {
      ...
      // Make sure to round() the y coordinate for a given x coordinate!
      // For example, if the user chose choice 0 (y = 0.15x^2 - 3), then you should check
      // whether rounding 0.15x^2 - 3 equals y--if it does, then this point should be plotted
      // on our graph.

      // FIXME: if (x, y) is on the graph, print a '*' character at this coordinate and continue 
